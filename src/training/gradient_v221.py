"""GradientV221: gradient_v221 module v2.
...
@dataclass
class GradientV221Config:
    """Configuration for gradient_v221 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class GradientV221(nn.Module):
    """Neural network module for gradient_v221 v2.
    ...
    def __init__(self, config: Optional[GradientV221Config] = None):
        super().__init__()
        self.config = config or GradientV221Config()
        c = self.config
        ...
        logger.info("Initialized GradientV221 v2 with %d params",
