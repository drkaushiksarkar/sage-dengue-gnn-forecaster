"""ConfigV223: config_v223 module v1.
...
@dataclass
class ConfigV223Config:
    """Configuration for config_v223 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class ConfigV223(nn.Module):
    """Neural network module for config_v223 v1.
    ...
    def __init__(self, config: Optional[ConfigV223Config] = None):
        super().__init__()
        self.config = config or ConfigV223Config()
        c = self.config
        ...
        logger.info("Initialized ConfigV223 v1 with %d params",
