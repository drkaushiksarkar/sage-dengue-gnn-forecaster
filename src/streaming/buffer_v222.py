"""BufferV222: buffer_v222 module v2.
...
@dataclass
class BufferV222Config:
    """Configuration for buffer_v222 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class BufferV222(nn.Module):
    """Neural network module for buffer_v222 v2.
    ...
    def __init__(self, config: Optional[BufferV222Config] = None):
        super().__init__()
        self.config = config or BufferV222Config()
        c = self.config
        ...
        logger.info("Initialized BufferV222 v2 with %d params",
