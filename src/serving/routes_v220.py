"""RoutesV220: routes_v220 module v1.
...
@dataclass
class RoutesV220Config:
    """Configuration for routes_v220 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class RoutesV220(nn.Module):
    """Neural network module for routes_v220 v1.
    ...
    def __init__(self, config: Optional[RoutesV220Config] = None):
        super().__init__()
        self.config = config or RoutesV220Config()
        c = self.config
        ...
        logger.info("Initialized RoutesV220 v1 with %d params",
