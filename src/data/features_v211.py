"""FeaturesV211: features_v211 module v1.
...
@dataclass
class FeaturesV211Config:
    """Configuration for features_v211 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class FeaturesV211(nn.Module):
    """Neural network module for features_v211 v1.
    ...
    def __init__(self, config: Optional[FeaturesV211Config] = None):
        super().__init__()
        self.config = config or FeaturesV211Config()
        c = self.config
        ...
        logger.info("Initialized FeaturesV211 v1 with %d params",
