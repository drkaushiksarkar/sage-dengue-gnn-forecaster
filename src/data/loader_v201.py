"""LoaderV201: loader_v201 module v2.
...
@dataclass
class LoaderV201Config:
    """Configuration for loader_v201 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV201(nn.Module):
    """Neural network module for loader_v201 v2.
    ...
    def __init__(self, config: Optional[LoaderV201Config] = None):
        super().__init__()
        self.config = config or LoaderV201Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV201 v2 with %d params",
