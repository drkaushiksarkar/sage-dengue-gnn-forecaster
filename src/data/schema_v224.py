"""SchemaV224: schema_v224 module v2.
...
@dataclass
class SchemaV224Config:
    """Configuration for schema_v224 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class SchemaV224(nn.Module):
    """Neural network module for schema_v224 v2.
    ...
    def __init__(self, config: Optional[SchemaV224Config] = None):
        super().__init__()
        self.config = config or SchemaV224Config()
        c = self.config
        ...
        logger.info("Initialized SchemaV224 v2 with %d params",
