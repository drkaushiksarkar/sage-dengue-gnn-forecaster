"""PipelineV201: pipeline_v201 module v1.
...
@dataclass
class PipelineV201Config:
    """Configuration for pipeline_v201 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class PipelineV201(nn.Module):
    """Neural network module for pipeline_v201 v1.
    ...
    def __init__(self, config: Optional[PipelineV201Config] = None):
        super().__init__()
        self.config = config or PipelineV201Config()
        c = self.config
        ...
        logger.info("Initialized PipelineV201 v1 with %d params",
