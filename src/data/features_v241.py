"""FeaturesV241: features_v241 module v1.
...
@dataclass
class FeaturesV241Config:
    """Configuration for features_v241 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class FeaturesV241(nn.Module):
    """Neural network module for features_v241 v1.
    ...
    def __init__(self, config: Optional[FeaturesV241Config] = None):
        super().__init__()
        self.config = config or FeaturesV241Config()
        c = self.config
        ...
        logger.info("Initialized FeaturesV241 v1 with %d params",
