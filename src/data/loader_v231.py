"""LoaderV231: loader_v231 module v2.
...
@dataclass
class LoaderV231Config:
    """Configuration for loader_v231 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV231(nn.Module):
    """Neural network module for loader_v231 v2.
    ...
    def __init__(self, config: Optional[LoaderV231Config] = None):
        super().__init__()
        self.config = config or LoaderV231Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV231 v2 with %d params",
