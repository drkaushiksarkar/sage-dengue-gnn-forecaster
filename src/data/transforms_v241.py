"""TransformsV241: transforms_v241 module v2.
...
@dataclass
class TransformsV241Config:
    """Configuration for transforms_v241 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class TransformsV241(nn.Module):
    """Neural network module for transforms_v241 v2.
    ...
    def __init__(self, config: Optional[TransformsV241Config] = None):
        super().__init__()
        self.config = config or TransformsV241Config()
        c = self.config
        ...
        logger.info("Initialized TransformsV241 v2 with %d params",
